...
(ns dynamic-critical-path.core
  ...
  (:gen-class
   :methods [#^{:static true} [dynamicCriticalPath [java.util.Map java.util.List] java.util.Set]]))
...
(defn -dynamicCriticalPath
  [v-weights edges]
  (dynamic-critical-path (generateDAG v-weights edges)))
